...
ax.add_patch(patches.Rectangle((0, steel_thickness), 10, zn_thickness, color=zn_color))  # Bottom Zn layer
...
bg_color = 'white'
bg_alpha = 0.5
ax.add_patch(patches.Rectangle((2, steel_thickness + zn_thickness + gpe_thickness / 2 - 0.2), 6, 0.6, color=bg_color, alpha=bg_alpha))  # Gel Polymer Electrolyte
...
plt.text(text_offset_x, steel_thickness + zn_thickness / 2, 'Zn', ha='center', va='center', fontsize=figure_text_size)
plt.text(text_offset_x, steel_thickness / 2, 'Stainless Steel', ha='center', va='center', fontsize=figure_text_size)

# Add yellow dots to represent ions, ensuring they don't overlap the Zn layers
num_dots = 100
dot_color = 'yellow'
