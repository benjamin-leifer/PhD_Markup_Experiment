...
total_height = 2 * steel_thickness + gpe_thickness
...
ax.add_patch(patches.Rectangle((0, steel_thickness), 10, gpe_thickness, color=electrolyte_color))

# Add transparent square behind the "Gel Polymer Electrolyte" text
bg_color = 'white'
bg_alpha = 0.5
ax.add_patch(patches.Rectangle((1.5, steel_thickness + gpe_thickness / 2 - 0.2), 7, 0.6, color=bg_color, alpha=bg_alpha))  # Gel Polymer Electrolyte
...
plt.text(text_offset_x, steel_thickness + gpe_thickness / 2, 'Gel Polymer Electrolyte', ha='center', va='center', fontsize=figure_text_size)
plt.text(text_offset_x, steel_thickness / 2, 'Stainless Steel', ha='center', va='center', fontsize=figure_text_size)

# Add yellow dots to represent ions, ensuring they don't overlap the Stainless Steel layers
num_dots = 100
dot_color = 'yellow'
for i in range(num_dots):
    x = np.random.uniform(0, 10)
    y = np.random.uniform(steel_thickness, steel_thickness + gpe_thickness)
...
plt.text(offset, steel_thickness + gpe_thickness / 2, '~0.7 mm', ha='center', va='center', fontsize=side_text_size)
