...
class ComparisonTab(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self.left_panel.pack(
            side=tk.LEFT, fill=tk.BOTH, expand=False, padx=10, pady=10, ipadx=5, ipady=5
        )
        ...
        ttk.Label(selection_frame, text="Available Tests:").pack(
        ...
        self.test_listbox = tk.Listbox(
            test_list_frame, selectmode=tk.EXTENDED, height=10
        )
        self.test_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        test_scrollbar = ttk.Scrollbar(
            test_list_frame, orient=tk.VERTICAL, command=self.test_listbox.yview
        )
        ...
        ttk.Label(selection_frame, text="Selected for Comparison:").pack(
            anchor=tk.W, padx=5, pady=5
        )
        ...
        self.selected_listbox = tk.Listbox(
            selected_list_frame, selectmode=tk.EXTENDED, height=10
        )
        ...
        ttk.Button(btn_frame, text="Add →", command=self.add_selected_tests).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(btn_frame, text="← Remove", command=self.remove_selected_tests).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(btn_frame, text="Clear All", command=self.clear_selected_tests).pack(
        ...
        ttk.Label(self.options_frame, text="Plot Type:").grid(
        ...
        self.cycle_entry = ttk.Entry(
            self.options_frame, width=10, textvariable=self.dqdv_cycle_var
        )
        ...
        ttk.Button(
            ...
            command=self.export_comparison_data,
        ).pack(fill=tk.X, padx=5, pady=5)
    ...
    def refresh_samples(self):
        ...
        try:
            ...
            self.main_app.log_message(
                f"Loaded {len(sample_names)} samples for comparison"
            )
        ...
        try:
            ...
            for test in tests:
                self.test_listbox.insert(
                    tk.END, f"{test.name} ({test.cycle_count} cycles)"
                )
    ...
    def add_selected_tests(self):
        ...
        for i in selected_indices:
            ...
            if test_label not in [
                self.selected_listbox.get(j)
                for j in range(self.selected_listbox.size())
            ]:
    ...
    def remove_selected_tests(self):
        ...
        self.main_app.log_message(
            f"Removed {len(selected_indices)} tests from comparison"
        )
        ...
        def comparison_thread():
            try:
                ...
                if not cycles_data:
                    messagebox.showinfo(
                        "No Data",
                        "Could not retrieve cycle data for any of the selected tests.",
                    )
                ...
                if plot_type == "capacity_vs_cycle":
                    self.plot_capacity_vs_cycle(
                        cycles_data, test_info, show_box, box_only
                    )
                ...
                self.main_app.log_message(
                    f"Generated comparison for {len(cycles_data)} tests"
                )
                ...
                self.main_app.log_message(
                    f"Error generating comparison: {str(e)}", logging.ERROR
                )
    ...
    def plot_capacity_vs_cycle(
        self, cycles_data, test_info, show_box=False, box_only=False
    ):
        ...
        if show_box:
            ...
            for idx in range(max_cycle):
                vals = [
                    data["cycles"][idx]["discharge_capacity"]
    ...
    def plot_coulombic_efficiency(self, cycles_data, test_info):
        ...
        for test_id, cycle_data in cycles_data.items():
            ...
            cycle_nums = [c["cycle_index"] for c in cycle_data["cycles"]]
            ce_values = [
                c["coulombic_efficiency"] * 100 for c in cycle_data["cycles"]
        ...
        ax.set_ylim(
            min(
                90,
                min(
                    min([c["coulombic_efficiency"] * 100 for c in data["cycles"]])
                    for data in cycles_data.values()
                )
                - 2,
            ),
    ...
    def plot_statistical_comparison(self, cycles_data, test_info):
        ...
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(
            2, 2, figsize=(8, 6), num=self.fig.number
        )
        ...
        for test_id, cycle_data in cycles_data.items():
            ...
            if cycles:
                ...
                retentions.append(
                    cycles[-1]["discharge_capacity"]
                    / cycles[0]["discharge_capacity"]
                    * 100
                )
                avg_ce.append(
                    np.mean([c["coulombic_efficiency"] * 100 for c in cycles])
                )
    ...
    def update_statistics_view(self, cycles_data, test_info):
        ...
        if cycles_data:
            ...
            for test_id, cycle_data in cycles_data.items():
                ...
                if cycles:
                    ...
                    if cycle_count > 1 and initial_cap > 0:
                        fade_rate = ((initial_cap - final_cap) / initial_cap) * (
                            100 / cycle_count
                        )
                    else:
                        fade_rate = 0
            ...
            self.stats_text.insert(
                tk.END,
                f"  Range: {min(initial_caps):.2f} - {max(initial_caps):.2f}\n\n",
            )
            ...
            self.stats_text.insert(
                tk.END, f"  Range: {min(final_caps):.2f} - {max(final_caps):.2f}\n\n"
            )
            ...
            self.stats_text.insert(
                tk.END, f"  Range: {min(retentions):.2f} - {max(retentions):.2f}\n\n"
            )

            # Average Coulombic Efficiency
            self.stats_text.insert(
                tk.END, "Average Coulombic Efficiency (%):\n", "subheading"
            )
            self.stats_text.insert(tk.END, f"  Mean: {np.mean(avg_ces):.2f}\n")
            self.stats_text.insert(tk.END, f"  Std Dev: {np.std(avg_ces):.2f}\n")
            self.stats_text.insert(
                tk.END, f"  Range: {min(avg_ces):.2f} - {max(avg_ces):.2f}\n\n"
            )

            # Fade Rate
            self.stats_text.insert(
                tk.END, "Average Fade Rate (%/cycle):\n", "subheading"
            )
            ...
            self.stats_text.insert(
                tk.END,
                f"  Range: {min(avg_fade_rates):.4f} - {max(avg_fade_rates):.4f}\n\n",
            )
            ...
            self.stats_text.insert(
                tk.END, "Ranked by Capacity Retention:\n", "subheading"
            )
            retention_ranks = sorted(
                zip(test_names, retentions), key=lambda x: x[1], reverse=True
            )
            ...
            self.stats_text.insert(
                tk.END, "Ranked by Coulombic Efficiency:\n", "subheading"
            )
            ce_ranks = sorted(
                zip(test_names, avg_ces), key=lambda x: x[1], reverse=True
            )
    ...
    def export_comparison_data(self):
        ...
        try:
                    ...
                    rows.append(
                        {
                            ...
                            "Coulombic Efficiency (%)": cycle["coulombic_efficiency"]
                            * 100,
                        }
                    )
            ...
            messagebox.showinfo(
                "Export Complete", f"Comparison data saved to {file_path}"
            )
    ...
    def generate_comparison_report(self):
        ...
        if not self.comparison_data:
            messagebox.showinfo(
                "No Data", "No comparison data available for reporting."
            )
            return
        ...
        try:
            ...
            def report_thread():
                try:
                    ...
                    report_file = report.generate_comparison_report(
                        test_objects, filename=file_path
                    )
                    ...
                    self.main_app.log_message(
                        f"Generated comparison report: {report_file}"
                    )
                    ...
                    self.main_app.queue.put(
                        {"type": "status", "text": f"Error generating report: {str(e)}"}
                    )
            ...
            self.main_app.log_message(
                f"Error setting up report generation: {str(e)}", logging.ERROR
            )
            messagebox.showerror("Error", f"Error generating report: {str(e)}")
