...
class EISTab(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        ttk.Label(self.file_frame, text="File:").grid(
        ...
        ttk.Label(self.file_frame, text="Associate with Sample:").grid(
            row=1, column=0, sticky=tk.W, padx=5, pady=5
        )
        self.file_sample_combobox = ttk.Combobox(self.file_frame, width=20)
        self.file_sample_combobox.grid(
            row=1, column=1, columnspan=2, sticky=tk.W + tk.E, padx=5, pady=5
        )

        self.import_btn = ttk.Button(
            self.file_frame, text="Import", command=self.import_file
        )
        self.import_btn.grid(
            row=2, column=0, columnspan=3, padx=5, pady=5, sticky=tk.E + tk.W
        )

        # Database selection frame (initially hidden)
        self.db_frame = ttk.Frame(data_source_frame)

        ttk.Label(self.db_frame, text="Sample:").grid(
            row=0, column=0, sticky=tk.W, padx=5, pady=5
        )
        ...
        ttk.Button(self.db_frame, text="Refresh", command=self.refresh_samples).grid(
            row=0, column=2, padx=5, pady=5
        )

        ttk.Label(self.db_frame, text="EIS Test:").grid(
            row=1, column=0, sticky=tk.W, padx=5, pady=5
        )
        self.test_combobox = ttk.Combobox(self.db_frame, width=20)
        self.test_combobox.grid(
            row=1, column=1, columnspan=2, sticky=tk.W + tk.E, padx=5, pady=5
        )

        self.load_btn = ttk.Button(
            self.db_frame, text="Load Data", command=self.load_eis_data
        )
        self.load_btn.grid(
            row=2, column=0, columnspan=3, padx=5, pady=5, sticky=tk.E + tk.W
        )
        ...
        if self.has_impedance:
            ...
            ttk.Label(fitting_frame, text="Circuit:").grid(
                row=0, column=0, sticky=tk.W, padx=5, pady=5
            )
        ...
        # If impedance.py is not available, show a warning
            ...
            warning_frame = ttk.LabelFrame(
                self.left_frame, text="Circuit Fitting Unavailable"
            )
        ...
        table_scrollbar = ttk.Scrollbar(
            self.data_table_frame, orient=tk.VERTICAL, command=self.data_table.yview
        )
        ...
        results_scrollbar = ttk.Scrollbar(
            self.results_text, orient=tk.VERTICAL, command=self.results_text.yview
        )
        ...
        if self.has_impedance:
            ...
            params_scrollbar = ttk.Scrollbar(
                params_inner_frame, orient=tk.VERTICAL, command=self.params_table.yview
            )
    ...
    def refresh_file_samples(self):
        ...
        try:
            ...
            self.main_app.log_message(
                f"Error loading samples for file association: {str(e)}", logging.ERROR
            )
    ...
    def refresh_samples(self):
        ...
        try:
            ...
            self.main_app.log_message(
                f"Loaded {len(sample_names)} samples for EIS analysis"
            )
        except Exception as e:
        ...
        try:
            ...
            for test_ref in sample.tests:
                ...
                # Also check custom_data for EIS data
                    ...
                    test
                    and hasattr(test, "custom_data")
                    and "eis_data" in test.custom_data
                ):
                    tests.append(test)
            ...
            if test_names:
                ...
                self.main_app.log_message(
                    f"No EIS tests found for sample {sample_name}"
                )

            self.main_app.log_message(
                f"Loaded {len(tests)} EIS tests for sample {sample_name}"
            )
        except Exception as e:
    ...
    def browse_file(self):
        ...
        file_path = filedialog.askopenfilename(
            title="Select EIS Data File", filetypes=filetypes
        )
    ...
    def import_file(self):
        ...
        self.main_app.update_status(
            f"Importing EIS file: {os.path.basename(file_path)}..."
        )
        ...
        threading.Thread(
            target=self._import_file_thread, args=(file_path, sample_id), daemon=True
        ).start()
    ...
    def _import_file_thread(self, file_path, sample_id):
        ...
        try:
            ...
            self.main_app.log_message(
                f"Imported EIS data from {os.path.basename(file_path)}"
            )
            ...
            self.main_app.log_message(
                f"Error importing EIS file: {str(e)}", logging.ERROR
            )
    ...
    def load_eis_data(self):
        ...
        if not sample_name or not test_name:
            messagebox.showinfo(
                "Selection Required", "Please select a sample and test."
            )
            return
        ...
        try:
            ...
            threading.Thread(
                target=self._load_eis_data_thread, args=(str(test.id),), daemon=True
            ).start()

        except Exception as e:
            self.main_app.log_message(
                f"Error loading EIS data: {str(e)}", logging.ERROR
            )
    ...
    def preprocess_data(self):
        ...
        try:
            ...
            self.current_eis_data["Z_phase"] = np.arctan2(z_imag_proc, z_real_proc) * (
                180 / np.pi
            )
            ...
            self.main_app.log_message(
                f"Error preprocessing data: {str(e)}", logging.ERROR
            )
            messagebox.showerror(
                "Preprocessing Error", f"Error preprocessing data: {str(e)}"
            )
    ...
    def plot_drt(self):
        ...
        try:
            ...
            messagebox.showerror(
                "DRT Error",
                f"Error plotting Distribution of Relaxation Times: {str(e)}",
            )
    ...
    def fit_circuit(self):
        ...
        if self.custom_var.get():
            ...
            if not circuit_string:
                messagebox.showwarning(
                    "Missing Circuit", "Please enter a custom circuit string."
                )
                return
        else:
        ...
        threading.Thread(
            target=self._fit_circuit_thread, args=(circuit_string,), daemon=True
        ).start()
    ...
    def _fit_circuit_thread(self, circuit_string):
        ...
        try:
            ...
            fit_result = eis.fit_circuit_model(
                frequency, z_real, z_imag, circuit_string
            )
            ...
            self.main_app.log_message(
                f"Fitted circuit: {circuit_string}, R² = {fit_result['r_squared']:.4f}"
            )
        ...
        try:
            ...
            if char_freqs["characteristic_frequencies"]:
                self.results_text.insert(
                    tk.END, "Characteristic Frequencies\n", "heading"
                )
            ...
            self.main_app.log_message(
                f"Error computing characteristic frequencies: {str(e)}", logging.WARNING
            )
    ...
    def update_parameters_table(self):
        ...
        try:
            ...
            for name, param_data in params.items():
                self.params_table.insert(
                    "",
                    "end",
                    values=(
                        name,
                        (
                            f"{param_data['value']:.6e}"
            ...
            self.main_app.log_message(
                f"Error updating parameters table: {str(e)}", logging.ERROR
            )
    ...
    def generate_report(self):
        ...
        threading.Thread(
            target=self._generate_report_thread, args=(file_path,), daemon=True
        ).start()
    ...
    def _generate_report_thread(self, file_path):
        ...
        try:
            ...
            if messagebox.askyesno(
                "Report Generated", f"EIS report saved to {report_file}. Open now?"
            ):
                import webbrowser
            ...
            self.main_app.log_message(
                f"Error generating EIS report: {str(e)}", logging.ERROR
            )
            messagebox.showerror(
                "Report Error", f"Error generating EIS report: {str(e)}"
            )
