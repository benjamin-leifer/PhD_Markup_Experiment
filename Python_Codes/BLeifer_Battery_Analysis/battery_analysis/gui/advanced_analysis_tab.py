...
class AdvancedAnalysisTab(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        selection_frame = ttk.LabelFrame(self.top_frame, text="Sample and Test Selection")
        ...
        for i, (text, value) in enumerate(analyses):
            ttk.Radiobutton(
                analysis_frame, text=text, variable=self.analysis_type_var, value=value,
        ...
        ttk.Label(self.dqdv_options, text="Cycle Number:").grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)
        ...
        ttk.Label(self.dqdv_options, text="Smoothing:").grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)
        ...
        ttk.Label(self.dqdv_options, text="Window Size:").grid(row=2, column=0, padx=5, pady=5, sticky=tk.W)
        ...
        self.results_scrollbar = ttk.Scrollbar(self.results_text, command=self.results_text.yview)
    ...
    def refresh_samples(self):
        ...
        try:
            ...
            self.main_app.log_message(f"Loaded {len(sample_names)} samples for advanced analysis")
    ...
    def on_sample_selected(self, event):
        ...
        try:
            ...
            self.main_app.log_message(f"Loaded {len(tests)} tests for advanced analysis of sample {sample_name}")
        except Exception as e:
    ...
    def on_test_selected(self, event):
        ...
        try:
            ...
            self.main_app.log_message(f"Selected test {test_name} for advanced analysis")
    ...
    def on_analysis_type_changed(self):
        ...
        if analysis_type == "dqdv":
            ...
            ttk.Label(frame, text="Cycle Number:").grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)
            self.cycle_entry = ttk.Entry(frame, width=10)
            self.cycle_entry.insert(0, "1")
            self.cycle_entry.grid(row=0, column=1, padx=5, pady=5)

            ttk.Label(frame, text="Smoothing:").grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)
            self.smooth_var = tk.BooleanVar(value=True)
            self.smooth_check = ttk.Checkbutton(frame, variable=self.smooth_var)
            self.smooth_check.grid(row=1, column=1, padx=5, pady=5)

            ttk.Label(frame, text="Window Size:").grid(row=2, column=0, padx=5, pady=5, sticky=tk.W)
            ...
            ttk.Label(frame, text="EOL Capacity (%):").grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)
            self.eol_entry = ttk.Entry(frame, width=10)
            self.eol_entry.insert(0, "80")
            self.eol_entry.grid(row=0, column=1, padx=5, pady=5)

            ttk.Label(frame, text="Models to Try:").grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)

            self.linear_var = tk.BooleanVar(value=True)
            self.linear_check = ttk.Checkbutton(frame, text="Linear", variable=self.linear_var)
            self.linear_check.grid(row=1, column=1, padx=5, pady=5, sticky=tk.W)

            self.power_var = tk.BooleanVar(value=True)
            self.power_check = ttk.Checkbutton(frame, text="Power Law", variable=self.power_var)
            self.power_check.grid(row=2, column=1, padx=5, pady=5, sticky=tk.W)

            self.exp_var = tk.BooleanVar(value=True)
            self.exp_check = ttk.Checkbutton(frame, text="Exponential", variable=self.exp_var)
            ...
            ttk.Label(frame, text="Metric:").grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)
            ...
            ttk.Label(frame, text="Detection Threshold (σ):").grid(row=4, column=0, padx=5, pady=5, sticky=tk.W)
            ...
            ttk.Label(frame, text="Cell Weight (g):").grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)
            ...
            ttk.Label(frame, text="Cell Volume (cm³):").grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)
            ...
            ttk.Label(frame, text="Note: Enter 0 if unknown").grid(row=2, column=0, columnspan=2, padx=5, pady=5)
            ...
            ttk.Label(frame, text="Clustering Method:").grid(row=0, column=1, padx=5, pady=5, sticky=tk.W)
            ...
            ttk.Label(frame, text="Number of Clusters:").grid(row=5, column=0, padx=5, pady=5, sticky=tk.W)
    ...
    def _run_analysis_thread(self, analysis_type):
        ...
        try:
            ...
            self.main_app.log_message(f"Error in {analysis_type} analysis: {str(e)}", logging.ERROR)
            messagebox.showerror("Analysis Error", f"Error running {analysis_type} analysis: {str(e)}")
    ...
    def _run_dqdv_analysis(self):
        ...
        try:
            ...
            self.main_app.log_message(f"dQ/dV analysis complete for cycle {cycle_number}")

        except Exception as e:
            self.main_app.log_message(f"Error in dQ/dV analysis: {str(e)}", logging.ERROR)
            raise
    ...
    def plot_dqdv_results(self):
        ...
        ax1.plot(self.current_dqdv_data['v_centers'], self.current_dqdv_data['dq_dv'], '-', linewidth=2)
        ax1.set_xlabel('Voltage (V)')
        ax1.set_ylabel('dQ/dV (mAh/V)')
        ax1.set_title(f'Differential Capacity Analysis - Cycle {self.current_dqdv_data["cycle"]}')
        ax1.grid(True, linestyle='--', alpha=0.7)

        # Find peaks
        from scipy.signal import find_peaks
        peaks, _ = find_peaks(self.current_dqdv_data['dq_dv'], height=0.1 * max(self.current_dqdv_data['dq_dv']),
                              distance=50)

        # Highlight peaks
        for peak_idx in peaks:
            ax1.plot(self.current_dqdv_data['v_centers'][peak_idx],
                     self.current_dqdv_data['dq_dv'][peak_idx],
                     'ro', markersize=8)
            ax1.annotate(f"{self.current_dqdv_data['v_centers'][peak_idx]:.3f} V",
        ...
        ax2.plot(self.current_dqdv_data['capacity'], self.current_dqdv_data['voltage'], '-')
    ...
    def update_dqdv_results_text(self):
        ...
        # Add title
        self.results_text.insert(tk.END, "Differential Capacity Analysis Results\n\n", "title")

        # Test and cycle info
        self.results_text.insert(tk.END, f"Sample: {self.current_sample.name}\n", "heading")
        self.results_text.insert(tk.END, f"Test: {self.current_test.name}\n", "heading")
        self.results_text.insert(tk.END, f"Cycle: {self.current_dqdv_data['cycle']}\n\n", "heading")

        # Peak information
        self.results_text.insert(tk.END, f"Detected Peaks (Phase Transitions)\n", "heading")
    ...
    def _run_fade_analysis(self):
        ...
        self.main_app.log_message(f"Running capacity fade analysis for test {self.current_test.name}")

        try:
            # Run capacity fade analysis
            self.current_fade_analysis = advanced_analysis.capacity_fade_analysis(test_id)
            ...
            self.main_app.log_message(f"Error in capacity fade analysis: {str(e)}", logging.ERROR)
            raise
    ...
    def plot_fade_results(self):
        ...
        if 'fade_models' in self.current_fade_analysis:
            ...
            for model_name, model_data in fade_models.items():
                ...
                if model_name == 'linear' and 'params' in model_data:
                    a, b = model_data['params']['slope'], model_data['params']['intercept']
                    x_fit = np.linspace(min(cycles), max(cycles) * 2, 100)
                    y_fit = a * x_fit + b
                    ax1.plot(x_fit, y_fit, '-', label=f'Linear (R²={model_data["r_squared"]:.3f})')

                elif model_name == 'power' and 'params' in model_data:
                    a, b, c = model_data['params']['a'], model_data['params']['b'], model_data['params']['c']
                    x_fit = np.linspace(min(cycles), max(cycles) * 2, 100)
                    y_fit = a * np.power(x_fit, b) + c
                    ax1.plot(x_fit, y_fit, '-', label=f'Power (R²={model_data["r_squared"]:.3f})')

                elif model_name == 'exponential' and 'params' in model_data:
                    a, b, c = model_data['params']['a'], model_data['params']['b'], model_data['params']['c']
                    x_fit = np.linspace(min(cycles), max(cycles) * 2, 100)
                    y_fit = a * np.exp(b * x_fit) + c
                    ax1.plot(x_fit, y_fit, '-', label=f'Exponential (R²={model_data["r_squared"]:.3f})')
        ...
        if self.current_fade_analysis['predicted_eol_cycle']:
            ...
            ax2.annotate(f"EOL at cycle {int(eol_cycle)}", (eol_cycle, 80),
            ...
            if self.current_fade_analysis['confidence']:
                ...
                ax2.annotate(f"Confidence: {conf:.1f}%", (eol_cycle, 80),
    ...
    def update_fade_results_text(self):
        ...
        self.results_text.insert(tk.END, f"Sample: {self.current_sample.name}\n", "heading")
        self.results_text.insert(tk.END, f"Test: {self.current_test.name}\n", "heading")
        self.results_text.insert(tk.END, f"Cycles: {self.current_fade_analysis['cycle_count']}\n\n", "heading")

        # Basic metrics
        self.results_text.insert(tk.END, "Capacity Metrics\n", "heading")
        self.results_text.insert(tk.END,
                                 f"Initial Capacity: {self.current_fade_analysis['initial_capacity']:.2f} mAh\n")
        self.results_text.insert(tk.END, f"Final Capacity: {self.current_fade_analysis['final_capacity']:.2f} mAh\n")
        self.results_text.insert(tk.END,
        ...
        # Model information
        if 'fade_models' in self.current_fade_analysis and len(self.current_fade_analysis['fade_models']) > 0:
            self.results_text.insert(tk.END, "Fade Models\n", "heading")

            for model_name, model_data in self.current_fade_analysis['fade_models'].items():
                if 'error' in model_data:
                    self.results_text.insert(tk.END, f"{model_data['name']}: Failed to fit ({model_data['error']})\n")
                    continue

                self.results_text.insert(tk.END, f"{model_data['name']} Model\n", "subheading")
                self.results_text.insert(tk.END, f"R² Value: {model_data['r_squared']:.4f}\n")

                # Parameters
                self.results_text.insert(tk.END, "Parameters:\n")
                for param_name, param_value in model_data['params'].items():
                    self.results_text.insert(tk.END, f"  {param_name}: {param_value:.6e}\n")

                # EOL prediction
                if model_data['eol_cycle'] is not None:
                    self.results_text.insert(tk.END, f"Predicted EOL Cycle (80%): {int(model_data['eol_cycle'])}\n")
            ...
            if self.current_fade_analysis['best_model']:
                ...
                self.results_text.insert(tk.END, f"R²: {best_model_data['r_squared']:.4f}\n")
        ...
        if fade_rate < 0.01:
            self.results_text.insert(tk.END, "Very low fade rate: <0.01% per cycle. Excellent cycling stability.\n")
        elif fade_rate < 0.05:
            self.results_text.insert(tk.END, f"Low fade rate: {fade_rate:.4f}% per cycle. Good cycling stability.\n")
        elif fade_rate < 0.1:
            self.results_text.insert(tk.END,
        ...
        if self.current_fade_analysis['best_model']:
            ...
            if best_model == 'linear':
                self.results_text.insert(tk.END, "Linear fade model fits best: Constant capacity loss per cycle.\n")
        ...
        if self.current_fade_analysis['predicted_eol_cycle']:
            ...
            if eol <= completed:
                self.results_text.insert(tk.END, f"Cell has already reached end-of-life criterion (80% capacity).\n")
            else:
                self.results_text.insert(tk.END, f"Projected to reach 80% capacity at cycle {eol} " +
                                         f"({eol - completed} more cycles from current state).\n")

                if self.current_fade_analysis['confidence']:
                    conf = self.current_fade_analysis['confidence'] * 100
                    if conf > 80:
                        self.results_text.insert(tk.END, f"High prediction confidence: {conf:.1f}%\n")
                    elif conf > 50:
                        self.results_text.insert(tk.END, f"Medium prediction confidence: {conf:.1f}%\n")
                    else:
                        self.results_text.insert(tk.END,
    ...
    def _run_anomaly_detection(self):
        ...
        try:
            ...
            anomaly_results = advanced_analysis.detect_anomalies(test_id, metric, threshold)
            ...
            self.main_app.log_message(f"Error in anomaly detection: {str(e)}", logging.ERROR)
            raise
    ...
    def plot_anomaly_results(self, anomaly_results, metric):
        ...
        if anomaly_cycles:
            ax.plot(anomaly_cycles, anomaly_values, 'ro', markersize=10,
                    label='Anomalies', zorder=2)

            # Annotate anomalies
            for cycle, value in zip(anomaly_cycles, anomaly_values):
                ax.annotate(f"Cycle {cycle}", (cycle, value),
                            xytext=(10, 10), textcoords='offset points',
                            arrowprops=dict(arrowstyle='->', lw=1.5))
    ...
    def update_anomaly_results_text(self, anomaly_results, metric):
        ...
        self.results_text.insert(tk.END, f"Analyzed Metric: {metric_name}\n\n", "heading")
        ...
        if anomaly_results['anomaly_count'] == 0:
            ...
            self.results_text.insert(tk.END, f"Total Anomalies: {anomaly_results['anomaly_count']}\n\n")

            # Table header
            self.results_text.insert(tk.END, "Cycle\tValue\tMethod\tSignificance\n", "table_header")
        ...
        if anomaly_results['anomaly_count'] == 0:
            ...
            z_score_anomalies = [a for a in anomaly_results['anomalies'] if a['detection_method'] == 'z-score']
            mavg_anomalies = [a for a in anomaly_results['anomalies'] if a['detection_method'] == 'moving_average']
            ...
            self.results_text.insert(tk.END, "- Measurement errors or equipment issues\n")
            self.results_text.insert(tk.END, "- Temperature fluctuations during testing\n")
            self.results_text.insert(tk.END, "- Cell internal short circuits or other failures\n")
            ...
            self.results_text.insert(tk.END, "- Investigate cycles with anomalous behavior\n")
            self.results_text.insert(tk.END, "- Consider excluding anomalous cycles when modeling capacity fade\n")
            self.results_text.insert(tk.END, "- Check testing equipment and conditions for consistency\n")
    ...
    def _run_energy_analysis(self):
        ...
        try:
            ...
            if weight or volume:
                ...
                if 'initial_discharge_energy' in energy_results and energy_results['initial_discharge_energy']:
            ...
            self.main_app.log_message(f"Error in energy analysis: {str(e)}", logging.ERROR)
            raise
    ...
    def plot_energy_results(self, energy_results):
        ...
        for c in cycle_data['cycles']:
            if 'discharge_energy' in c and c['discharge_energy'] is not None:
                ...
                discharge_energy.append(c['discharge_capacity'] * 3.7 / 1000)  # Convert to Wh

            if 'energy_efficiency' in c and c['energy_efficiency'] is not None:
                energy_efficiency.append(c['energy_efficiency'] * 100)  # Convert to percentage
            else:
        ...
        if discharge_energy:
            ax1.annotate(f"Initial: {discharge_energy[0]:.3f} Wh",
                         xy=(cycles[0], discharge_energy[0]),
                         xytext=(10, 10), textcoords='offset points',
                         arrowprops=dict(arrowstyle='->', lw=1.5))

            ax1.annotate(f"Final: {discharge_energy[-1]:.3f} Wh",
    ...
    def update_energy_results_text(self, energy_results):
        ...
        if initial_charge is not None:
            self.results_text.insert(tk.END, f"Initial Charge Energy: {initial_charge:.3f} Wh\n")

        final_charge = energy_results.get('final_charge_energy')
        if final_charge is not None:
            self.results_text.insert(tk.END, f"Final Charge Energy: {final_charge:.3f} Wh\n")

        initial_discharge = energy_results.get('initial_discharge_energy')
        if initial_discharge is not None:
            self.results_text.insert(tk.END, f"Initial Discharge Energy: {initial_discharge:.3f} Wh\n")

        final_discharge = energy_results.get('final_discharge_energy')
        if final_discharge is not None:
            self.results_text.insert(tk.END, f"Final Discharge Energy: {final_discharge:.3f} Wh\n")

        avg_charge = energy_results.get('avg_charge_energy')
        if avg_charge is not None:
            self.results_text.insert(tk.END, f"Average Charge Energy: {avg_charge:.3f} Wh\n")

        avg_discharge = energy_results.get('avg_discharge_energy')
        if avg_discharge is not None:
            self.results_text.insert(tk.END, f"Average Discharge Energy: {avg_discharge:.3f} Wh\n")
        ...
        if avg_efficiency is not None:
            self.results_text.insert(tk.END, f"Average Energy Efficiency: {avg_efficiency * 100:.2f}%\n")

        retention = energy_results.get('energy_retention')
        if retention is not None:
            self.results_text.insert(tk.END, f"Energy Retention: {retention * 100:.2f}%\n")

        fade_rate = energy_results.get('energy_fade_rate_pct_per_cycle')
        if fade_rate is not None:
            self.results_text.insert(tk.END, f"Energy Fade Rate: {fade_rate:.4f}% per cycle\n")
        ...
        if energy_density:
            ...
            if grav_density is not None:
                self.results_text.insert(tk.END, f"Gravimetric Energy Density: {grav_density:.1f} Wh/kg\n")

            vol_density = energy_density.get('volumetric')
            if vol_density is not None:
                self.results_text.insert(tk.END, f"Volumetric Energy Density: {vol_density:.1f} Wh/L\n")
        ...
        try:
            ...
            if len(sample_tests) < 5:
                ...
                for sample in all_samples:
                    sample_test_ids = [str(t.id) for t in models.TestResult.objects(sample=sample)]
            ...
            self.main_app.log_message(f"Error in clustering analysis: {str(e)}", logging.ERROR)
            raise
    ...
    def plot_clustering_results(self, cluster_results):
        ...
        try:
            ...
            for cluster_id in clusters:
                cluster_tests = [test for test in test_info if test.get('cluster') == cluster_id]
                ...
                # Plot this cluster
                ax.scatter(x_vals, y_vals, color=colors[cluster_id % len(colors)],
            ...
            ax.text(0.5, 0.5, f"Could not create cluster visualization: {str(e)}",
    ...
    def update_clustering_results_text(self, cluster_results):
        ...
        self.results_text.insert(tk.END, f"Number of Clusters: {cluster_results['n_clusters']}\n")
        self.results_text.insert(tk.END, f"Metrics Used: {', '.join(cluster_results['metrics_used'])}\n")
        self.results_text.insert(tk.END, f"Total Tests: {len(cluster_results['tests'])}\n\n")

        # Cluster details
        self.results_text.insert(tk.END, "Cluster Details\n", "heading")

        for cluster_id, tests in cluster_results['clusters'].items():
            self.results_text.insert(tk.END, f"Cluster {cluster_id} ({len(tests)} tests)\n", "subheading")
        ...
        self.results_text.insert(tk.END, "• Identifying groups of cells with similar performance\n")
        self.results_text.insert(tk.END, "• Finding outliers that perform differently from the majority\n")
        self.results_text.insert(tk.END, "• Categorizing cells by their performance characteristics\n")
        self.results_text.insert(tk.END, "• Identifying manufacturing batches with consistent properties\n\n")

        self.results_text.insert(tk.END, "Next steps:\n")
        self.results_text.insert(tk.END, "• Investigate what makes each cluster unique\n")
        self.results_text.insert(tk.END, "• Compare physical or chemical properties of cells in different clusters\n")
        self.results_text.insert(tk.END,
