...
def generate_plots(test_result):
    ...
    coulombic_efficiencies = [c.coulombic_efficiency * 100 for c in cycles]  # Convert to percentage

    # Plot 1: Capacity vs Cycle
    plt.figure(figsize=(8, 6))
    plt.plot(cycle_numbers, discharge_capacities, marker='o', linestyle='-',
             label='Discharge Capacity', color='blue')
    ...
    if test_result.capacity_retention is not None:
        ...
        plt.annotate(f"Capacity Retention: {retention_pct:.1f}%",
                     xy=(0.05, 0.05), xycoords='axes fraction',
                     bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="gray", alpha=0.8))

    # Save the plot as a temporary image
    capacity_plot_path = os.path.join(tempfile.gettempdir(), f"capacity_plot_{int(time.time())}.png")
    ...
    if test_result.avg_coulombic_eff is not None:
        ...
        plt.annotate(f"Average CE: {avg_ce:.2f}%",
                     xy=(0.05, 0.05), xycoords='axes fraction',
                     bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="gray", alpha=0.8))

    # Save the plot
    ce_plot_path = os.path.join(tempfile.gettempdir(), f"ce_plot_{int(time.time())}.png")
    ...
    if len(cycle_numbers) > 5:
        ...
        normalized_capacities = [cap / first_capacity * 100 for cap in discharge_capacities]
        ...
        # Save the plot
        norm_plot_path = os.path.join(tempfile.gettempdir(), f"norm_plot_{int(time.time())}.png")
...
def create_pdf_report(test_result, plot_paths, filename):
    ...
    # Title
    elements.append(Paragraph(f"Battery Test Report", styles['Title']))
    ...
    if hasattr(test_result.sample, 'nominal_capacity') and test_result.sample.nominal_capacity is not None:
        sample_data.append(["Nominal Capacity:", f"{test_result.sample.nominal_capacity:.2f} mAh"])
    ...
    test_data = [
        ...
        ["Test Date:", test_result.date.strftime("%Y-%m-%d %H:%M:%S") if test_result.date else "N/A"],
    ...
    if hasattr(test_result, 'upper_cutoff_voltage') and test_result.upper_cutoff_voltage is not None:
        test_data.append(["Upper Cutoff Voltage:", f"{test_result.upper_cutoff_voltage:.3f} V"])

    if hasattr(test_result, 'lower_cutoff_voltage') and test_result.lower_cutoff_voltage is not None:
        test_data.append(["Lower Cutoff Voltage:", f"{test_result.lower_cutoff_voltage:.3f} V"])
    ...
    if hasattr(test_result, 'discharge_rate') and test_result.discharge_rate is not None:
    ...
    metrics_data = [
        ...
        ["Average Coulombic Efficiency:", f"{test_result.avg_coulombic_eff * 100:.3f}%"]
    ]

    # Add energy efficiency if available
    if hasattr(test_result, 'avg_energy_efficiency') and test_result.avg_energy_efficiency is not None:
        metrics_data.append(["Average Energy Efficiency:", f"{test_result.avg_energy_efficiency * 100:.3f}%"])

    # Calculate capacity fade rate per cycle
    if test_result.cycle_count > 1 and test_result.initial_capacity > 0:
        fade_rate = ((test_result.initial_capacity - test_result.final_capacity) /
                     test_result.initial_capacity) * (100.0 / test_result.cycle_count)
    ...
    for i, plot_path in enumerate(plot_paths):
        ...
        if i == 0:
            ...
            elements.append(Paragraph("Coulombic Efficiency vs. Cycle Number", styles['Normal']))
        elif i == 2:
            elements.append(Paragraph("Normalized Capacity vs. Cycle Number", styles['Normal']))
        ...
        try:
            ...
            elements.append(Paragraph(f"Error including plot {i + 1}: {e}", styles['Normal']))
...
def generate_comparison_report(test_results, filename=None):
    ...
    # Title
    elements.append(Paragraph(f"Battery Test Comparison Report", styles['Title']))
    elements.append(Spacer(1, 0.1 * inch))
    ...
    # Save the plot
    comparison_plot_path = os.path.join(tempfile.gettempdir(), f"comparison_plot_{int(time.time())}.png")
    ...
    for i, (cycles, capacities, name) in enumerate(zip(cycle_nums, discharge_caps, sample_names)):
        first_capacity = capacities[0] if capacities else 1.0
        normalized = [cap / first_capacity * 100 for cap in capacities]

        plt.plot(cycles, normalized, marker='o', markersize=4,
    ...
    normalized_plot_path = os.path.join(tempfile.gettempdir(), f"normalized_comparison_{int(time.time())}.png")
    ...
    # Table header
    summary_data = [
        ["Sample", "Test", "Cycles", "Initial Cap. (mAh)", "Final Cap. (mAh)", "Retention (%)", "Avg. CE (%)"]
    ]
    ...
    elements.append(Paragraph("Normalized Discharge Capacity Comparison", styles['Normal']))
