...
try:
    ...
    HAS_IMPEDANCE = False
    warnings.warn("impedance.py package not found. Install with 'pip install impedance' for full EIS functionality.")
...
def parse_text_eis(file_path):
    ...
    for col in df.columns:
        col_str = str(col).lower()
        if ('real' in col_str or 're(z)' in col_str or 'z\'' in col_str or 'z_real' in col_str or
            'z real' in col_str) and 'imag' not in col_str:
            real_col = col
            break
    ...
    for col in df.columns:
        col_str = str(col).lower()
        if ('imag' in col_str or 'im(z)' in col_str or 'z\"' in col_str or 'z_imag' in col_str or
                'z imag' in col_str or '-im' in col_str):
            imag_col = col
            break
    ...
    if freq_col is None or real_col is None or imag_col is None:
        raise ValueError(f"Could not identify frequency, real, and imaginary columns in file: {file_path}")
...
def parse_biologic_eis(file_path):
    ...
    try:
        ...
        try:
            ...
            raise ImportError("Failed to parse BioLogic file. Consider installing 'galvani' package.") from e
        except Exception as bin_error:
            raise ValueError(f"Could not parse BioLogic file: {e}. Binary parsing also failed: {bin_error}")
...
def preprocess_eis_data(frequency, z_real, z_imag, f_min=None, f_max=None, inductive_filter=True):
    """
...
def validate_eis_data(frequency, z_real, z_imag):
    ...
    if HAS_IMPEDANCE:
        try:
            ...
            if avg_res > 0.05:
                results['warnings'].append(f"Poor Kramers-Kronig compliance (avg residual: {avg_res:.3f})")

        except Exception as e:
            results['warnings'].append(f"Could not perform Kramers-Kronig validation: {str(e)}")
...
def fit_circuit_model(frequency, z_real, z_imag, circuit_string, initial_guess=None, bounds=None):
    """
    ...
    if not HAS_IMPEDANCE:
        raise ImportError("impedance.py is required for circuit fitting. Install with 'pip install impedance'")
    ...
    try:
        ...
        r_squared = 1 - np.sum(np.abs(residuals) ** 2) / np.sum(np.abs((impedance - np.mean(impedance)) ** 2))
...
def compute_characteristic_frequencies(frequency, z_real, z_imag):
    ...
    peaks, peak_properties = find_peaks(neg_z_imag, height=0.01 * max(neg_z_imag), prominence=0.05 * max(neg_z_imag))
    ...
    if HAS_IMPEDANCE and len(frequency) > 20:
        try:
            from impedance.visualization import plot_distribution_of_relaxation_times
            ...
            frequencies_pred, gamma, tau, error, sigma = impedance.models.circuits.drt.calculate_drt(
                f, z, reg_method='ridge', penalty_method='derivative', M=1, rbf_type='cubic',
                sigma=0.05
            )
...
def extract_physical_parameters(fit_result):
    ...
    for r_param in [p for p in params if p.startswith('R') and p != 'R0']:
        ...
        for c_param in [p for p in params if p.startswith('C')]:
            if f"p({r_param},{c_param})" in circuit or f"p({c_param},{r_param})" in circuit:
                cap_param = c_param
...
def compare_eis_spectra(test_ids, apply_normalization=False):
    ...
    for data in eis_datasets:
        ...
        try:
            ...
            key_params.append({
                ...
                'basic_metrics': {
                    'R_hf': params.get('R_hf', {}).get('value') if not HAS_IMPEDANCE else params.get(
                        'electrolyte_resistance', {}).get('value'),
                    'R_lf': params.get('R_lf', {}).get('value') if not HAS_IMPEDANCE else None,
    ...
    if len(key_params) >= 2 and all('basic_metrics' in p for p in key_params):
        ...
        r_hf_values = [p['basic_metrics']['R_hf'] for p in key_params if p['basic_metrics'].get('R_hf') is not None]
...
def plot_nyquist(eis_data, ax=None, label=None, include_fit=False, highlight_frequencies=False):
    """
    ...
    if isinstance(eis_data, dict):
        ...
        raise ValueError("eis_data must be a dictionary with Z_real and Z_imag keys, or a tuple of (Z_real, Z_imag)")
    ...
    if highlight_frequencies and frequency is not None:
        ...
        highlight_freqs = [
            0.01, 0.1, 1, 10, 100, 1000, 10000, 100000  # Standard frequencies
        ]
...
def plot_bode(eis_data, ax=None, label=None, include_fit=False, phase_units='degrees'):
    ...
    if isinstance(eis_data, dict):
        ...
        raise ValueError("eis_data must be a dictionary with frequency, Z_real, and Z_imag keys")
...
def plot_drt(eis_data):
    ...
    """
    if not HAS_IMPEDANCE:
        raise ImportError("impedance.py is required for DRT analysis. Install with 'pip install impedance'")
    ...
    try:
        # Perform DRT analysis
        freq_pred, gamma, tau, error, sigma = impedance.models.circuits.drt.calculate_drt(
            frequency, z, reg_method='ridge', penalty_method='derivative', M=1, rbf_type='cubic',
            sigma=0.05
        )
        ...
        for peak_idx in peaks:
            ...
            ax.annotate(f"{tau[peak_idx]:.1e} s",
...
def generate_comprehensive_eis_report(eis_data, fit_results=None, filename=None):
    ...
    # Plot Nyquist
    plot_nyquist({'Z_real': eis_data['Z_real'], 'Z_imag': eis_data['Z_imag']}, ax=ax1, label='Data')
    ...
    if fit_results is not None and 'fitted_impedance' in fit_results:
        ...
        if 'circuit_string' in fit_results:
            ...
            ax2.text(0.05, 0.95, circuit_text, transform=ax2.transAxes,
                     verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
    else:
        ax2.text(0.5, 0.5, "No fit data available", transform=ax2.transAxes,
                 ha='center', va='center')
    ...
    if fit_results is not None and 'fitted_impedance' in fit_results:
        ...
        ax4.semilogx(eis_data['frequency'], -z_fit_phase, '--', color='red', label='Fit')
        ax4.legend()
    ...
    plt.figtext(0.5, 0.98, info_text, ha='center', va='top', fontsize=10,
