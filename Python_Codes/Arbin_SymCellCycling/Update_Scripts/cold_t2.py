...
plots_dir = os.path.join(base_dir, "plots_t4")
...
def plot_groups_by_alpha(groups, lookup, out_dir, cell_meta, electrolyte_colors, max_additive):
    ...
    # marker set for cell-level differentiation
    ...
    for alpha, entries in sorted(alpha_groups.items()):
        ...
        for cell_code, path in entries:
            ...
            # sparse markers so the curve stays clean
            if len(x_spec) > 1:
        ...
        if alpha in lookup.index:
            electrolyte_label = lookup.loc[alpha, "Electrolyte"]

        ax.set_title(f"{alpha} group — {electrolyte_label}")
...
def plot_dtf_dtfv_groups(groups, lookup, out_dir, cell_meta, electrolyte_colors, max_additive):
    ...
    for group_name, alpha_list in electrolyte_sets.items():
        ...
        ax.set_title(f"{group_name} group — DTF/DTFV discharge")
        ax.set_xlabel("Discharge Specific Capacity (mAh/g)")
        ax.set_ylabel("Voltage (V)")
        ax.set_ylim(0, 4.5)
        ax.set_xlim(-4, 160)
...
# --- main changes: pass temp_tag="-51" to only include -51C data ---
def main():
    ...
    for cell_code, paths in groups.items():
        ...
        if lookup_key in lookup.index:
            electrolyte_label = lookup.loc[lookup_key, "Electrolyte"]

        title_label = f"{cell_code}: - {electrolyte_label}"
    ...
    # by-alpha plots with consistent colors / widths
    plot_groups_by_alpha(groups, lookup, plots_dir, cell_meta, electrolyte_colors, max_additive)

    # DTF / DTFV grouped plots with same color/width rules
