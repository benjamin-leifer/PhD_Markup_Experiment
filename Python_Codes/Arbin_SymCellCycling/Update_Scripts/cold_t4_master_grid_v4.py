...
plots_dir = os.path.join(base_dir, "plots_t10")
...
# --- trial/replicate styling (suffix number in cell code, e.g., HU01/HU02/HU03) ---
# Used for consistent visual cues across the master grid and scatter plots.
TRIAL_MARKERS = {
    1: "o",   # first trial/replicate
    2: "s",   # second
    3: "^",   # third
}
TRIAL_LINE_ALPHA = {
...
RUN_MASTER_GRID_ALL = True
RUN_MASTER_GRID_BEST = True
RUN_FEC_SHIFT_PLOTS = True
RUN_VC_SHIFT_PLOTS = True
RUN_SCATTER_PLOTS = True

# Metric definitions for "best" scoring + scatter plots
METRIC_TARGET_VOLTAGE = 2.5  # capacity at this voltage (V) is used for metrics/selection
            ...
            for idx, (cell_code, paths) in enumerate(sorted(entries, key=lambda x: x[0])):
                ...
                for p in sorted(paths):
                    ...
                    ax.plot(
                        ...
                        markevery=markevery,
                        label=label,
                        alpha=cell_line_alpha.get(cc, 1.0),
                    )
                    label = None
                    plotted_any = True
...
def plot_fec_vc_master_grid(groups, lookup, out_dir, cell_meta, electrolyte_colors, max_additive,
    ...
    # Trial/replicate visual cues (consistent across panels).
    # We interpret the "trial number" as the numeric suffix in the cell code (e.g., HU01->1, HU02->2, HU03->3).
    markers = ["o", "s", "D", "^", "v", "P", "X", "*", "h", "<", ">"]

    def _trial_number(cc: str):
        digits = "".join(ch for ch in str(cc)[2:] if ch.isdigit())
        if not digits:
            return None
        try:
            n = int(digits)
    ...
    def _trial_marker(cc: str) -> str:
        t = _trial_number(cc)
        if t is None:
            return "o"
        return TRIAL_MARKERS.get(t, markers[(t - 1) % len(markers)])

    def _trial_alpha(cc: str) -> float:
        t = _trial_number(cc)
        return TRIAL_LINE_ALPHA.get(t, 1.0)

    cell_marker = {cc: _trial_marker(cc) for cc in groups.keys()}
    cell_line_alpha = {cc: _trial_alpha(cc) for cc in groups.keys()}
            ...
            for ref_cc in ref_cells:
                ...
                for p in sorted(paths):
                    ...
                    ax.plot(
                        ...
                        markevery=markevery,
                        label=label,
                        alpha=0.85 * cell_line_alpha.get(ref_cc, 1.0),
                    )
                    label = None
...
# ---------- metrics + effect plots (FEC shift / VC shift / scatter) ----------
def trial_number_from_cell_code(cell_code: str):
    """Return numeric suffix as int (HU01->1, HU02->2, ...)."""
    digits = "".join(ch for ch in str(cell_code)[2:] if ch.isdigit())
    if not digits:
        return None
    try:
        n = int(digits)
...
def pick_best_path_for_cell(paths, lookup, score_voltage=2.5):
    """Pick the single file for this cell with max capacity at score_voltage."""
    best_p = None
    best_score = -np.inf
    for p in sorted(paths):
        try:
            ...
            continue
        cap = _capacity_at_voltage(Q, V, target_voltage=score_voltage)
        if np.isfinite(cap) and cap > best_score:
            best_score = cap
            best_p = p
    return best_p, best_score


def avg_discharge_voltage(Q_mAh_g, V):
    """Average discharge voltage over the full Q-span: ∫V dQ / (Q_end - Q_start)."""
    if Q_mAh_g is None or V is None:
        return np.nan
    Q = np.asarray(Q_mAh_g, dtype=float)
    VV = np.asarray(V, dtype=float)
    m = np.isfinite(Q) & np.isfinite(VV)
    Q, VV = Q[m], VV[m]
    if len(Q) < 2:
        return np.nan
    order = np.argsort(Q)
    ...
    if dQ <= 0:
        return np.nan
    area = np.trapz(VV, Q)
    return float(area / dQ)


def initial_discharge_voltage(Q_mAh_g, V):
    """Voltage at the start of discharge (first point at Q>=0 if available)."""
    if Q_mAh_g is None or V is None:
        return np.nan
    Q = np.asarray(Q_mAh_g, dtype=float)
    VV = np.asarray(V, dtype=float)
    ...
    Q, VV = Q[order], VV[order]
    if np.any(Q >= 0):
        idx = int(np.argmax(Q >= 0))  # first True
    else:
        idx = 0
    return float(VV[idx])


def build_discharge_metrics(groups, lookup, cell_meta,
                            fec_levels=(1, 2, 5, 10), vc_levels=(0, 1, 2),
                            score_voltage=2.5,
                            reference_alphas=("FA", "EC")):
    """One row per cell_code, using its *best* -51C discharge file (by cap@score_voltage)."""
    rows = []
    for cell_code, paths in groups.items():
        if not paths:
            continue
        best_path, best_score = pick_best_path_for_cell(paths, lookup, score_voltage=score_voltage)
        if best_path is None:
            continue

        try:
            Q, V, _ = load_discharge_curve(best_path, lookup=lookup)
        except Exception:
            continue
        ...
        initV = initial_discharge_voltage(Q, V)
        cap_2p5 = _capacity_at_voltage(Q, V, target_voltage=2.5)
        avgV = avg_discharge_voltage(Q, V)

        anode, cathode = get_anode_cathode(cell_code, lookup)

        rows.append({
            "cell_code": cell_code,
            "alpha": str(cell_code)[:2],
            "trial": trial_number_from_cell_code(cell_code),
            "electrolyte": electrolyte,
            "fec": fec,
            "vc": vc,
            "anode": anode,
            "cathode": cathode,
            "best_path": best_path,
            f"cap_at_{score_voltage:.2f}V_mAh_g": best_score,
            "cap_at_2.50V_mAh_g": cap_2p5,
            "init_V": initV,
            "avg_discharge_V": avgV,
        })
...
def _filter_grid_conditions(df, fec_levels, vc_levels):
    d = df.copy()
    d = d[np.isfinite(d["fec"]) & np.isfinite(d["vc"])]
    d = d[d["fec"].isin(list(fec_levels)) & d["vc"].isin(list(vc_levels))]
    return d


def plot_fec_shift_metrics(df_metrics, out_dir, fec_levels=(1, 2, 5, 10), vc_levels=(0, 1, 2)):
    """Best method (recommended): metric-vs-FEC lines, one line per VC level."""
    os.makedirs(out_dir, exist_ok=True)
    d = _filter_grid_conditions(df_metrics, fec_levels, vc_levels)

    # Capacity @ 2.5 V vs FEC, colored by VC
    fig, ax = plt.subplots(figsize=(7.5, 5.0))
    for vc in vc_levels:
        sub = d[d["vc"] == vc]
        stats = sub.groupby("fec")["cap_at_2.50V_mAh_g"].agg(["mean", "std"]).reindex(list(fec_levels))
        ax.errorbar(list(fec_levels), stats["mean"], yerr=stats["std"], marker="o", linewidth=1.5, label=f"VC {vc}%")
    ax.set_xlabel("FEC (wt%)")
    ax.set_ylabel("Capacity at 2.5 V (mAh/g)")
    ax.set_title("FEC shift effect (metrics): Capacity@2.5 V")
    ax.legend(fontsize="small", loc="best")
    ax.grid(True, alpha=0.2)
    fig.tight_layout()
    out_path = os.path.join(out_dir, "FEC_shift_capacity_at_2p5V.png")
    fig.savefig(out_path, dpi=300)
    plt.close(fig)
    print(f"Saved: {out_path}")

    # Avg discharge V vs FEC, colored by VC
    fig, ax = plt.subplots(figsize=(7.5, 5.0))
    for vc in vc_levels:
        sub = d[d["vc"] == vc]
        stats = sub.groupby("fec")["avg_discharge_V"].agg(["mean", "std"]).reindex(list(fec_levels))
        ax.errorbar(list(fec_levels), stats["mean"], yerr=stats["std"], marker="o", linewidth=1.5, label=f"VC {vc}%")
    ax.set_xlabel("FEC (wt%)")
    ax.set_ylabel("Average discharge voltage (V)")
    ax.set_title("FEC shift effect (metrics): Avg discharge V")
    ax.legend(fontsize="small", loc="best")
    ax.grid(True, alpha=0.2)
    fig.tight_layout()
    out_path = os.path.join(out_dir, "FEC_shift_avg_discharge_V.png")
    fig.savefig(out_path, dpi=300)
    plt.close(fig)
    print(f"Saved: {out_path}")


def plot_vc_shift_metrics(df_metrics, out_dir, fec_levels=(1, 2, 5, 10), vc_levels=(0, 1, 2)):
    """Best method (recommended): metric-vs-VC lines, one line per FEC level."""
    os.makedirs(out_dir, exist_ok=True)
    d = _filter_grid_conditions(df_metrics, fec_levels, vc_levels)

    # Capacity @ 2.5 V vs VC, colored by FEC
    fig, ax = plt.subplots(figsize=(7.5, 5.0))
    for fec in fec_levels:
        sub = d[d["fec"] == fec]
        stats = sub.groupby("vc")["cap_at_2.50V_mAh_g"].agg(["mean", "std"]).reindex(list(vc_levels))
        ax.errorbar(list(vc_levels), stats["mean"], yerr=stats["std"], marker="o", linewidth=1.5, label=f"FEC {fec}%")
    ax.set_xlabel("VC (wt%)")
    ax.set_ylabel("Capacity at 2.5 V (mAh/g)")
    ax.set_title("VC shift effect (metrics): Capacity@2.5 V")
    ax.legend(fontsize="small", loc="best")
    ax.grid(True, alpha=0.2)
    fig.tight_layout()
    out_path = os.path.join(out_dir, "VC_shift_capacity_at_2p5V.png")
    fig.savefig(out_path, dpi=300)
    plt.close(fig)
    print(f"Saved: {out_path}")

    # Avg discharge V vs VC, colored by FEC
    fig, ax = plt.subplots(figsize=(7.5, 5.0))
    for fec in fec_levels:
        sub = d[d["fec"] == fec]
        stats = sub.groupby("vc")["avg_discharge_V"].agg(["mean", "std"]).reindex(list(vc_levels))
        ax.errorbar(list(vc_levels), stats["mean"], yerr=stats["std"], marker="o", linewidth=1.5, label=f"FEC {fec}%")
    ax.set_xlabel("VC (wt%)")
    ax.set_ylabel("Average discharge voltage (V)")
    ax.set_title("VC shift effect (metrics): Avg discharge V")
    ax.legend(fontsize="small", loc="best")
    ax.grid(True, alpha=0.2)
    fig.tight_layout()
    out_path = os.path.join(out_dir, "VC_shift_avg_discharge_V.png")
    fig.savefig(out_path, dpi=300)
    plt.close(fig)
    print(f"Saved: {out_path}")


def plot_best_curves_for_fec_shift(df_metrics, lookup, out_dir,
                                  fec_levels=(1, 2, 5, 10), vc_levels=(0, 1, 2)):
    """Optional: Representative *best* curves showing FEC shift (one figure per VC)."""
    os.makedirs(out_dir, exist_ok=True)
    d = _filter_grid_conditions(df_metrics, fec_levels, vc_levels)

    # choose best cell per (fec, vc) by cap@2.5V
    idx = d.groupby(["fec", "vc"])["cap_at_2.50V_mAh_g"].idxmax()
    best = d.loc[idx].set_index(["fec", "vc"])

    for vc in vc_levels:
        fig, ax = plt.subplots(figsize=(7.5, 5.0))
        for fec in fec_levels:
            if (fec, vc) not in best.index:
                continue
            row = best.loc[(fec, vc)]
            p = row["best_path"]
            try:
                ...
                continue
            ax.plot(Q, V, linewidth=2.0, label=f"FEC {fec}% (cell {row['cell_code']})")
        ax.set_xlabel("Discharge Specific Capacity (mAh/g)")
        ax.set_ylabel("Voltage (V)")
        ax.set_title(f"Best curves: FEC shift at fixed VC={vc}%")
        ax.set_ylim(0, 4.5)
        ax.set_xlim(-4, 160)
        ax.grid(True, alpha=0.2)
        ax.legend(fontsize="x-small", loc="best")
        fig.tight_layout()
        out_path = os.path.join(out_dir, f"BestCurves_FECshift_VC{vc}.png")
        fig.savefig(out_path, dpi=300)
        plt.close(fig)
        print(f"Saved: {out_path}")


def plot_best_curves_for_vc_shift(df_metrics, lookup, out_dir,
                                 fec_levels=(1, 2, 5, 10), vc_levels=(0, 1, 2)):
    """Optional: Representative *best* curves showing VC shift (one figure per FEC)."""
    os.makedirs(out_dir, exist_ok=True)
    d = _filter_grid_conditions(df_metrics, fec_levels, vc_levels)
    ...
    for fec in fec_levels:
        ...
        for vc in vc_levels:
            ...
            try:
                ...
                continue
            ax.plot(Q, V, linewidth=2.0, label=f"VC {vc}% (cell {row['cell_code']})")
        ax.set_xlabel("Discharge Specific Capacity (mAh/g)")
        ax.set_ylabel("Voltage (V)")
        ax.set_title(f"Best curves: VC shift at fixed FEC={fec}%")
        ax.set_ylim(0, 4.5)
        ax.set_xlim(-4, 160)
        ax.grid(True, alpha=0.2)
        ax.legend(fontsize="x-small", loc="best")
        fig.tight_layout()
        out_path = os.path.join(out_dir, f"BestCurves_VCshift_FEC{fec}.png")
        fig.savefig(out_path, dpi=300)
        plt.close(fig)
        print(f"Saved: {out_path}")


def plot_scatter_initV_vs_metrics(df_metrics, out_dir):
    """Two scatter plots:
      (1) init_V vs capacity@2.5V
      (2) init_V vs avg discharge V
    Markers encode trial/replicate number.
    """
    os.makedirs(out_dir, exist_ok=True)

    d = df_metrics.copy()
    d = d[np.isfinite(d["init_V"]) & np.isfinite(d["cap_at_2.50V_mAh_g"]) & np.isfinite(d["avg_discharge_V"])]

    # init_V vs cap@2.5
    fig, ax = plt.subplots(figsize=(7.5, 5.0))
    for trial in sorted([t for t in d["trial"].dropna().unique() if t is not None]):
        sub = d[d["trial"] == trial]
        ax.scatter(sub["init_V"], sub["cap_at_2.50V_mAh_g"],
                   marker=TRIAL_MARKERS.get(int(trial), "o"),
                   s=40, label=f"Trial {int(trial)}", alpha=0.8)
    # include non-numbered trials, if any
    sub = d[d["trial"].isna()]
    if len(sub) > 0:
        ax.scatter(sub["init_V"], sub["cap_at_2.50V_mAh_g"], marker="o", s=40, label="No trial #", alpha=0.6)
    ax.set_xlabel("Initial discharge potential (V)")
    ax.set_ylabel("Capacity at 2.5 V (mAh/g)")
    ax.set_title("Initial potential vs capacity@2.5 V")
    ax.grid(True, alpha=0.2)
    ax.legend(fontsize="small", loc="best")
    fig.tight_layout()
    out_path = os.path.join(out_dir, "scatter_initV_vs_capAt2p5V.png")
    fig.savefig(out_path, dpi=300)
    plt.close(fig)
    print(f"Saved: {out_path}")

    # init_V vs avg discharge V
    fig, ax = plt.subplots(figsize=(7.5, 5.0))
    for trial in sorted([t for t in d["trial"].dropna().unique() if t is not None]):
        sub = d[d["trial"] == trial]
        ax.scatter(sub["init_V"], sub["avg_discharge_V"],
                   marker=TRIAL_MARKERS.get(int(trial), "o"),
                   s=40, label=f"Trial {int(trial)}", alpha=0.8)
    sub = d[d["trial"].isna()]
    if len(sub) > 0:
        ax.scatter(sub["init_V"], sub["avg_discharge_V"], marker="o", s=40, label="No trial #", alpha=0.6)
    ax.set_xlabel("Initial discharge potential (V)")
    ax.set_ylabel("Average discharge voltage (V)")
    ax.set_title("Initial potential vs average discharge voltage")
    ax.grid(True, alpha=0.2)
    ax.legend(fontsize="small", loc="best")
    fig.tight_layout()
    out_path = os.path.join(out_dir, "scatter_initV_vs_avgDischargeV.png")
    fig.savefig(out_path, dpi=300)
    plt.close(fig)
    print(f"Saved: {out_path}")
...
def main():
    ...
    # ---------- metrics + shift plots + scatters ----------
    metrics_dir = os.path.join(plots_dir, "metrics_outputs")
    os.makedirs(metrics_dir, exist_ok=True)

    df_metrics = build_discharge_metrics(
        groups, lookup, cell_meta,
        fec_levels=GRID_FEC_LEVELS,
        vc_levels=GRID_VC_LEVELS,
        score_voltage=METRIC_TARGET_VOLTAGE,
        reference_alphas=GRID_REFERENCE_ALPHAS,
    )
    metrics_csv = os.path.join(metrics_dir, "discharge_metrics_bestfile_per_cell.csv")
    df_metrics.to_csv(metrics_csv, index=False)
    print(f"Saved metrics CSV: {metrics_csv}")

    if RUN_FEC_SHIFT_PLOTS:
        fec_dir = os.path.join(metrics_dir, "FEC_shift")
        plot_fec_shift_metrics(df_metrics, fec_dir, fec_levels=GRID_FEC_LEVELS, vc_levels=GRID_VC_LEVELS)
        # optional representative curves:
        plot_best_curves_for_fec_shift(df_metrics, lookup, os.path.join(fec_dir, "best_curves"),
                                       fec_levels=GRID_FEC_LEVELS, vc_levels=GRID_VC_LEVELS)

    if RUN_VC_SHIFT_PLOTS:
        vc_dir = os.path.join(metrics_dir, "VC_shift")
        plot_vc_shift_metrics(df_metrics, vc_dir, fec_levels=GRID_FEC_LEVELS, vc_levels=GRID_VC_LEVELS)
        # optional representative curves:
        plot_best_curves_for_vc_shift(df_metrics, lookup, os.path.join(vc_dir, "best_curves"),
                                      fec_levels=GRID_FEC_LEVELS, vc_levels=GRID_VC_LEVELS)

    if RUN_SCATTER_PLOTS:
        scatter_dir = os.path.join(metrics_dir, "scatter")
        plot_scatter_initV_vs_metrics(df_metrics, scatter_dir)
