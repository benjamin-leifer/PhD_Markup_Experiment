...
def get_file_max_cycle_fast(xlsx_path: Path) -> int:
    """
    Very fast pass to read only the 'Cycle' column from sheet index 1 (2nd sheet)
    via openpyxl's read_only mode and return the max cycle number in the file.
    Falls back to 0 if cannot determine.
    """
    try:
        import openpyxl  # uses the same engine you already rely on
    except Exception:
        # Fallback: slower but safe if openpyxl isn't available
        try:
            df = pd.read_excel(xlsx_path, sheet_name=1, usecols=lambda c: "cycle" in c.lower())
            if df.empty:
                return 0
            cycle_col = df.columns[0]
            return int(pd.to_numeric(df[cycle_col], errors="coerce").max(skipna=True) or 0)
        except Exception:
            return 0

    try:
        wb = openpyxl.load_workbook(xlsx_path, read_only=True, data_only=True)
        ws = wb.worksheets[1]  # sheet index 1 (second sheet)
    except Exception:
        return 0

    # Find header row and the 'Cycle' column index
    header = None
    cycle_col_idx = None
    for i, row in enumerate(ws.iter_rows(min_row=1, max_row=5, values_only=True), start=1):
        if row and any(v is not None for v in row):
            # Heuristic: header row usually near the top and contains "Cycle"
            header = [str(v).strip() if v is not None else "" for v in row]
            # normalize & find cycle column
            norm = [re.sub(r"[^a-z0-9]", "", h.lower()) for h in header]
            for j, h in enumerate(norm):
                if h.startswith("cycleindex") or h.startswith("cyclenumber") or h == "cycle":
                    cycle_col_idx = j
                    break
            if cycle_col_idx is not None:
                break
    ...
    max_cycle = 0
    # Iterate remaining rows in that column only
    for row in ws.iter_rows(min_row=(i + 1 if header else 1), values_only=True):
        if row is None:
            continue
        val = row[cycle_col_idx] if cycle_col_idx < len(row) else None
        if val is None:
            continue
        try:
            c = int(float(val))
...
def build_imported_max_index(out_csv: Path) -> dict[tuple[str, str], int]:
    """
    Returns {(Cell, Dataset): max_cycle_imported} from the existing CSV.
    Backward compatible if 'Dataset' is missing (uses '*').
    """
    idx: dict[tuple[str, str], int] = {}
    if not out_csv.exists():
        return idx
    try:
        df = pd.read_csv(out_csv, usecols=lambda c: c in {"Cell", "Dataset", "Cycle"}, engine="python", on_bad_lines="skip")
        if "Dataset" not in df.columns:
            df["Dataset"] = "*"
        df["Cell"] = df["Cell"].astype(str).str.upper()
        df["Dataset"] = df["Dataset"].astype(str)
        df["Cycle"] = pd.to_numeric(df["Cycle"], errors="coerce").astype("Int64")
        grp = df.dropna(subset=["Cycle"]).groupby(["Cell", "Dataset"])["Cycle"].max()
        for (cell, dataset), mc in grp.items():
            idx[(cell, dataset)] = int(mc)
    except Exception as e:
        logging.exception("Failed to build imported max index from %s: %s", out_csv, e)
    return idx
...
def main():
    ...
    ap.add_argument("--root", type=str, default=r"C:\Users\benja\Downloads\Temp\C_10 Cycling\2025",
    ...
    imported_max = build_imported_max_index(out_csv)  # {(Cell, Dataset): max_cycle}
