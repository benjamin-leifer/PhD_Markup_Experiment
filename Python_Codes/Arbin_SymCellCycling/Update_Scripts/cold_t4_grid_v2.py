...
plots_dir = os.path.join(base_dir, "plots_t8")
...
def main():
    ...
    # --- NEW: FEC x VC grid plots (rows=FEC 1/2/5/10, cols=VC 0/1/2) ---
    grid_dir = os.path.join(plots_dir, "alpha_FECxVC_grids")

    # 1) All alphas, overlay all replicates (FA/EC included as references in every panel)
    plot_alpha_fec_vc_grid(groups, lookup, grid_dir, cell_meta=cell_meta, electrolyte_colors=electrolyte_colors,
                           alpha_select=None, selection="all", best_voltage=2.0, show_markers=True)

    # # 2) One alpha only (e.g., HU), overlay all replicates
    # # plot_alpha_fec_vc_grid(groups, lookup, grid_dir, cell_meta=cell_meta, electrolyte_colors=electrolyte_colors,
    # #                        alpha_select="HU", selection="all", show_markers=True)
    #
    # # 3) Global "best" grid (best cell per bin across all alphas; metric = capacity @ best_voltage)
    # # plot_alpha_fec_vc_grid(groups, lookup, grid_dir, cell_meta=cell_meta, electrolyte_colors=electrolyte_colors,
    # #                        alpha_select="best", selection="best", best_voltage=2.0, show_markers=True)


# ---------- FEC/VC parsing + grid plots ----------
def parse_fec_vc(
    electrolyte: str,
    allowed_fec=(1, 2, 5, 10),
    allowed_vc=(0, 1, 2),
):
    """
    Parse (FEC_wt%, VC_wt%) from electrolyte strings like:
      - DTF141     -> (1, 0)
      - DTF1410    -> (10, 0)
      - DTFV1411   -> (1, 1)
      - DTFV14102  -> (10, 2)

    Returns (fec, vc) as ints, or (None, None) if it can't be parsed (or if
    parsed values are not in allowed_fec/allowed_vc when provided).
    """
    if not electrolyte:
        return None, None

    e = str(electrolyte).upper().replace(" ", "")

    def digits_after(tag: str) -> str:
        if tag not in e:
            return ""
        tail = e.split(tag, 1)[1]
        return "".join(ch for ch in tail if ch.isdigit())

    # DTFV14(F,V)
    if "DTFV14" in e:
        digits = digits_after("DTFV14")
        if not digits:
            return None, None
        ...
        for i in range(1, len(digits)):
            try:
                ...
                continue
            if 0 <= F <= 20 and 0 <= V <= 20:
                candidates.append((F, V, i))

        if not candidates:
            return None, None

        # Prefer candidates that match the requested grid levels
        matches = [
            (F, V, i) for (F, V, i) in candidates
            if (allowed_fec is None or F in allowed_fec) and (allowed_vc is None or V in allowed_vc)
        ]
        pool = matches if matches else candidates

        # Choose the "most specific" split: largest F, then largest V, then longer F digits
        F, V, _ = sorted(pool, key=lambda t: (t[0], t[1], t[2]), reverse=True)[0]
        return F, V

    # DTF14(F)
    if "DTF14" in e:
        digits = digits_after("DTF14")
        if not digits:
            return None, None
        try:
            F = int(digits)
            ...
            return None, None
        V = 0
        if allowed_fec is not None and F not in allowed_fec:
            return None, None
        if allowed_vc is not None and V not in allowed_vc:
            return None, None
        return F, V
...
def _capacity_at_voltage(x, v, target_v=2.0):
    """
    Estimate the discharge capacity (x) at a given voltage (target_v)
    using linear interpolation along the discharge curve.

    Returns np.nan if the curve never reaches target_v.
    """
    if x is None or v is None or len(x) < 2 or len(v) < 2:
        return np.nan
    ...
    for i in range(len(v) - 1):
        v1, v2 = v[i], v[i + 1]
        if (v1 >= target_v and v2 <= target_v) or (v1 <= target_v and v2 >= target_v):
            if v2 == v1:
                return float(x[i])
            frac = (target_v - v1) / (v2 - v1)
            return float(x[i] + frac * (x[i + 1] - x[i]))

    return np.nan


def _best_score_for_cell(paths, lookup, target_v=2.0):
    """
    Score a cell by the max capacity it achieves at target_v (across all files).
    """
    best = np.nan
    for p in sorted(paths):
        try:
            x, y, _ = load_discharge_curve(p, lookup=lookup)
        except Exception:
            continue
        score = _capacity_at_voltage(x, y, target_v=target_v)
        if np.isnan(score):
            continue
...
def plot_alpha_fec_vc_grid(
    groups,
    lookup,
    out_dir,
    cell_meta=None,
    electrolyte_colors=None,
    fec_levels=(1, 2, 5, 10),
    vc_levels=(0, 1, 2),
    alpha_select=None,
    selection="all",   # "all" or "best"
    best_voltage=2.0,
    include_reference_alphas=("FA", "EC"),
    show_markers=True,
):
    """
    Make 4x3 grids (rows=FEC, cols=VC). Legends show CELL CODES.

    Modes:
      - alpha_select=None:          plot all alphas found in `groups`
      - alpha_select="HU":        plot only HU
      - alpha_select=["HU","HV"]: plot only those
      - alpha_select="best":      make ONE grid with the best-performing cell in each (FEC,VC)
                                   bin across ALL alphas (still includes FA/EC reference lines).

    selection:
      - "all": overlay all replicates in each bin
      - "best": within each alpha's grid, pick only the best replicate per bin (best_voltage metric)

    include_reference_alphas:
      Any cells starting with these alpha prefixes are plotted as references in EVERY subplot.
    """
    os.makedirs(out_dir, exist_ok=True)

    # --- collect reference cells (FA/EC) ---
    ref_cells = []
    for cell_code in sorted(groups.keys()):
        if any(cell_code.startswith(a) for a in include_reference_alphas):
            ref_cells.append(cell_code)

    # Cache reference curves (so we don't re-read xlsx 12x per figure)
    ref_curve_cache = defaultdict(list)  # cell_code -> list[(x,y)]
    for rc in ref_cells:
        for p in sorted(groups.get(rc, [])):
            try:
                x, y, _ = load_discharge_curve(p, lookup=lookup)
            except Exception:
                continue
            ref_curve_cache[rc].append((x, y))

    # Determine which "alphas" to plot
    all_alphas = sorted({c[:2] for c in groups.keys()})

    def _normalize_alpha_select(alpha_select_):
        if alpha_select_ is None:
            return all_alphas
        if isinstance(alpha_select_, str):
            if alpha_select_.lower() == "best":
                return ["best"]
            return [alpha_select_.strip()[:2]]
        # list/tuple/set
        return [str(a).strip()[:2] for a in alpha_select_]

    targets = _normalize_alpha_select(alpha_select)

    # helper: determine electrolyte for a cell_code
    def _get_electrolyte(cell_code: str) -> str:
        electrolyte = ""
        if cell_meta and cell_code in cell_meta:
            electrolyte = str(cell_meta[cell_code].get("electrolyte", "")).strip()
        if not electrolyte:
            electrolyte = get_electrolyte_name(cell_code, lookup)
        return electrolyte

    # helper: plot one grid given a list of candidate cell codes
    def _plot_one_grid(tag: str, candidate_cells: list[str], force_best=False):
        nrows, ncols = len(fec_levels), len(vc_levels)
        fig, axes = plt.subplots(
            nrows=nrows, ncols=ncols,
            figsize=(3.6 * ncols, 2.75 * nrows),
        ...
        # Map each candidate to a (fec,vc) bin
        bins = defaultdict(list)  # (fec,vc) -> [cell_code,...]
        for cell_code in candidate_cells:
            if cell_code in ref_cells:
                continue  # refs handled separately
            electrolyte = _get_electrolyte(cell_code)
            fec, vc = parse_fec_vc(electrolyte, allowed_fec=fec_levels, allowed_vc=vc_levels)
            if fec is None or vc is None:
                continue
            bins[(fec, vc)].append(cell_code)

        # Decide which cells to actually plot per bin
        best_cells = {}
        if force_best or selection.lower() == "best":
            # pre-score every candidate once
            score_cache = {}
            for (fec, vc), cell_list in bins.items():
                for cc in cell_list:
                    if cc not in score_cache:
                        score_cache[cc] = _best_score_for_cell(groups.get(cc, []), lookup, target_v=best_voltage)
                # pick best in this bin
                scored = [(cc, score_cache.get(cc, np.nan)) for cc in cell_list]
                scored = [(cc, sc) for cc, sc in scored if not np.isnan(sc)]
                if scored:
                    best_cc = sorted(scored, key=lambda t: t[1], reverse=True)[0][0]
                    best_cells[(fec, vc)] = [best_cc]
                else:
                    best_cells[(fec, vc)] = []
        else:
            best_cells = bins  # plot all
        ...
        # Plot data cells
        for (fec, vc), cell_list in best_cells.items():
            r = fec_levels.index(fec)
            c = vc_levels.index(vc)
            ax = axes[r, c]

            for idx, cell_code in enumerate(sorted(cell_list)):
                electrolyte = _get_electrolyte(cell_code)
                color = None
                if electrolyte_colors and electrolyte:
                    color = electrolyte_colors.get(electrolyte, None)

                label = cell_code  # <-- legends are cell codes
                if label in used_labels:
                    label = None
                else:
                    used_labels.add(label)
                ...
                for p in sorted(groups.get(cell_code, [])):
                    try:
                        x, y, _ = load_discharge_curve(p, lookup=lookup)
                    except Exception as exc:
                        print(f"Skipping {p} in {tag} grid: {exc}")
                        continue

                    if show_markers:
                        markevery = max(len(x) // 30, 1) if len(x) > 1 else 1
                        ax.plot(
                            x, y,
                            color=color,
                            linewidth=1.6,
                    ...
                    label = None  # only label first trace for this cell
                    plotted_any = True

            # Plot references in this panel (FA/EC), lightly styled
            # Only label them once per figure.
            for rc_idx, rc in enumerate(sorted(ref_cells)):
                rc_label = rc if rc not in used_labels else None
                if rc_label:
                    used_labels.add(rc_label)

                for (x_ref, y_ref) in ref_curve_cache.get(rc, []):
                    ax.plot(
                        x_ref, y_ref,
                        color="0.35",
                        linewidth=1.0,
        ...
        # Global labels
        try:
            fig.supxlabel("Discharge Specific Capacity (mAh/g)")
            fig.supylabel("Voltage (V)")
        except Exception:
            fig.text(0.5, 0.02, "Discharge Specific Capacity (mAh/g)", ha="center")
            fig.text(0.02, 0.5, "Voltage (V)", va="center", rotation="vertical")

        # Title
        if tag == "best":
            fig.suptitle(f"BEST grid (metric: capacity @ {best_voltage:.2f} V)", y=0.995)
            out_name = "BEST_FECxVC_grid.png"
        else:
            anode, cathode = get_anode_cathode(tag, lookup)
            chem = ""
            if cathode or anode:
                if cathode and anode:
                    chem = f"{cathode}|{anode}"
                elif cathode:
                    chem = cathode
                else:
                    chem = anode

            mode = "all replicates" if selection.lower() != "best" else f"best/bin @ {best_voltage:.2f} V"
            title_parts = [f"{tag} grid", mode]
            if chem:
                title_parts.append(chem)

            fig.suptitle(" — ".join(title_parts), y=0.995)
            out_name = f"{tag}_FECxVC_grid.png"
        ...
        if handles:
            fig.legend(handles, labels, fontsize="xx-small", ncol=4, loc="lower center", bbox_to_anchor=(0.5, 0.01))
    ...
    # Build candidate lists and generate grids
    for tgt in targets:
        if tgt == "best":
            # For the global best, force best/bin selection
            _plot_one_grid("best", list(groups.keys()), force_best=True)
        else:
            candidates = [c for c in sorted(groups.keys()) if c.startswith(tgt)]
            _plot_one_grid(tgt, candidates, force_best=False)
