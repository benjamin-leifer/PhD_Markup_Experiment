...
plots_dir = os.path.join(base_dir, "plots_t7")
...
def main():
    ...
    # --- NEW: per-alpha FEC x VC grid plots ---
    ...
    #                        fec_levels=(1, 2, 5, 10), vc_levels=(0, 1, 2), show_markers=False)
if __name__ == "__main__":
    main()



# ---------- FEC/VC parsing for grid plots ----------
def parse_fec_vc(electrolyte: str,
    ...
    (or if parsed values are not in allowed_fec/allowed_vc when provided).
    """
    if not electrolyte:
        return None, None
...
def plot_alpha_fec_vc_grid(groups, lookup, out_dir,
                           cell_meta=None,
                           ...
                           show_markers=False):
    """
    For each alpha (first two chars of cell code), make a 4x3 grid:
      - Rows:    FEC wt%  (ascending)
      - Columns: VC  wt%  (ascending)

    Each subplot overlays all cells within that alpha whose electrolyte parses
    to that (FEC, VC) coordinate.

    Notes:
      - Uses existing `load_discharge_curve()` normalization + voltage/capacity conventions.
      - Uses `get_display_label()` for legend labels.
    """
    os.makedirs(out_dir, exist_ok=True)

    # group cell codes by alpha
    alpha_to_cells = defaultdict(list)
    for cell_code in groups.keys():
        alpha_to_cells[cell_code[:2]].append(cell_code)

    # markers for distinguishability (optional)
    markers = ["o", "s", "D", "^", "v", "P", "X", "*", "h", "<", ">"]

    for alpha in sorted(alpha_to_cells.keys()):
        cell_codes = sorted(alpha_to_cells[alpha])
        ...
        # Plot each cell into its grid slot
        for idx, cell_code in enumerate(cell_codes):
            paths = groups.get(cell_code, [])
            electrolyte = ""
            if cell_meta and cell_code in cell_meta:
                electrolyte = cell_meta[cell_code].get("electrolyte", "")
            if not electrolyte:
                electrolyte = get_electrolyte_name(cell_code, lookup)
            ...
            for p in sorted(paths):
                try:
                    ...
                    print(f"Skipping {p} in {alpha} grid: {exc}")
                    continue
        ...
        # Row labels (FEC) and axis formatting
        for r, fec in enumerate(fec_levels):
            # Put the FEC label on the left-most subplot in the row
            axes[r, 0].annotate(
                f"FEC = {fec} wt%",
                xy=(-0.28, 0.5),
        ...
        # Title with chem if available
        anode, cathode = get_anode_cathode(alpha, lookup)
        chem = ""
        if cathode or anode:
            if cathode and anode:
        ...
        if handles:
            fig.legend(handles, labels, fontsize="xx-small", ncol=3, loc="lower center", bbox_to_anchor=(0.5, 0.01))
        ...
        out_path = os.path.join(out_dir, f"{alpha}_FECxVC_grid.png")
        fig.savefig(out_path, dpi=300)
        plt.close(fig)
        print(f"Saved {out_path}")
