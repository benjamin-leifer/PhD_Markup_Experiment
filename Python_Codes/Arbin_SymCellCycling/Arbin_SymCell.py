...
class Cell_Cycle:
    ...
    def get_c_rate(self):
        C_Rate = round(
            np.average(self.data["Current (A)"] / (self.mass * self.theoretical_cap)), 3
        )
        print("C Rate: ", C_Rate)
        print(C_Rate)
        print(type(C_Rate))
...
class arbin_import_Sym_Cell:
    ...
    def instantiate_cycle_list(self):
        ...
        self.cycles_objs = [
            ...
            for name, group in list(grouped_data)[:22]
        ]
    ...
    def plot_voltage_vs_time(self):
        ...
        ax1.plot(
            self.data["Test Time (s)"] / 3600, self.data["Voltage (V)"], label=self.name
        )
    ...
    def plot_voltage_vs_capacity(self, cycles="All", clean_filter=False):
        ...
        if clean_filter:
            ...
            for name, group in grouped_data:
                if (
                    group["Charge Capacity (Ah)"].max() / self.mass < 50
                    or group["Discharge Capacity (Ah)"].max() / self.mass < 50
                ):
                    pass
                else:
    ...
    def get_overpotentials(self):
        ...
        plt.scatter(
            self.cycle_Num_list[0 : len(self.overpotentials)],
            self.overpotentials,
            c=self.color,
            marker=self.shape,
            label=self.name + " Overpotentials",
        )
        # plt.show()
    ...
    def get_max_capacity_per_cycle(self):
        ...
        plt.scatter(
            self.cycle_Num_list[0 : len(self.max_capacity)],
            self.max_capacity,
            c=self.color,
            marker="*",
            label=self.name + " Charge Capacity",
        )
        # plt.show()
    ...
    def get_min_capacity_per_cycle(self):
        ...
        plt.scatter(
            self.cycle_Num_list[0 : len(self.max_dis_capacity)],
            self.max_dis_capacity,
            c=self.color,
            marker=self.shape,
            label=self.name + " Discharge Capacity",
        )
        # plt.show()
    ...
    def get_coulombic_efficiency(self):
        ...
        print(self.ce)
        plt.scatter(
            self.cycle_Num_list[0 : len(self.ce)],
            self.ce,
            c=self.color,
            marker=self.shape,
            label=self.name + " Coulombic Efficiency",
        )
        # plt.show()
    ...
    def plot_capacity_and_ce_vs_cycle(self):
        ...
        plt.title(
            "Capacity and Coulombic Efficiency vs. Cycle Number for %s" % self.name
        )
...
if __name__ == "__main__":
    ...
    #         title="Input", prompt="Please enter your text:"
    #     )
    #
    ...
    #     # cells.append(
    #     #     arbin_import_Sym_Cell(
    #     #         file,
    #     #         name=label_tag,
    #     #         mass=0.01430645161 / 1000,
    #     #         theoretical_cap=155,
    #     #         color=colors[i],
    #     #         shape="o",
    #     #     )
    ...
    for cell in cells:
        if cell.cycles < 1000:
            ...
            plt.savefig(
                cell.name + "_voltage_vs_time.png", dpi=500, bbox_inches="tight"
            )
            cell.plot_voltage_vs_capacity(clean_filter=False)
            plt.savefig(
                cell.name + "_voltage_vs_capacity.png", dpi=500, bbox_inches="tight"
            )
            plt.clf()
            ...
            # plt.show()
            plt.savefig(
                cell.name + "_coulombic_efficiency.png", dpi=500, bbox_inches="tight"
            )
