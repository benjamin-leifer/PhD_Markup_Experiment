...
# Main script
# Perform file and directory selection once
...
print(f"Selected Voltage Files: {voltage_files}")
print(f"Selected EIS Files: {eis_files}")
print(f"Save Directory: {save_dir}")
# Load voltage and EIS datasets into memory
print("Loading Voltage Datasets...")
# Load voltage datasets
voltage_datasets = []
for f in voltage_files:
    try:
        dataset = extract_voltage_capacity_from_file(f)
        if not dataset.empty:
            voltage_datasets.append(dataset)
        else:
            print(f"Warning: Failed to load voltage data from {f}. Skipping.")
    except Exception as e:
        print(f"Error loading voltage file {f}: {e}")

# Load EIS datasets
eis_datasets = []
for f in eis_files:
    try:
        dataset = read_eis_data(f)
        if dataset is not None and not dataset.empty:
            eis_datasets.append(dataset)
        else:
            print(f"Warning: Failed to load EIS data from {f}. Skipping.")
    except Exception as e:
        print(f"Error loading EIS file {f}: {e}")

# Ensure datasets are valid
if not voltage_datasets or not eis_datasets:
    raise ValueError("One or more datasets failed to load. Check your input files.")


# Process EIS voltages
eis_voltages = [
    [cycle_data["Ewe (V)"].iloc[0] for _, cycle_data in eis_data.groupby("Cycle Number")]
    for eis_data in eis_datasets
]


# Define colors and circuit
cycle_colors = plt.cm.viridis(np.linspace(0, 1, sum(len(eis["Cycle Number"].unique()) for eis in eis_datasets)))
# Fit EIS data for all datasets
circuit = 'R1-p(R2,CPE2)-p(R3,CPE3)-p(CPE4,R4)-W'
fitting_results = {idx: fit_eis_data_parallel(eis_data, circuit) for idx, eis_data in enumerate(eis_datasets)}
...
plot_voltage_capacity_multi(ax1, voltage_datasets, eis_voltages, cycle_colors)

# Subplot (b): Combined 3D Nyquist Plot
ax2 = fig.add_subplot(1, 2, 2, projection='3d')
plot_combined_3d_nyquist_with_fits_multi(ax2, eis_datasets, circuit)

# Save the figure
output_path = f"{save_dir}/combined_voltage_eis_plot.jpg"
plt.tight_layout()
