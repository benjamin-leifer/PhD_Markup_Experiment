...
import electrolyte_style_helpers_schemeA as esh
# =========================
# USER INPUT
# =========================
...
plots_dir = os.path.join(base_dir, "plots_-51C_combined_DTF")
...
FIG_W = 7.5 * FIG_SCALE
FIG_H = 6.0 * FIG_SCALE
LINE_WIDTH = 3.2 * FONT_SCALE
MARKER_SIZE = 9.0 * FONT_SCALE
...
def main():
    ...
    for cell_code in CELL_CODES:
        electrolyte = get_electrolyte(cell_code, lookup)
        ...
        for p in sorted(cell_files):
            try:
                ...
                st = esh.style_for_electrolyte(electrolyte)
                ax.plot(
                    Q,
                    V,
                    linewidth=max(LINE_WIDTH, st.get("linewidth", LINE_WIDTH)),
                    linestyle=st.get("linestyle", "-"),
                    marker=st.get("marker", "o"),
                    markersize=max(MARKER_SIZE, st.get("markersize", MARKER_SIZE)),
                    markerfacecolor=st.get("markerfacecolor", st.get("color", "#000000")),
                    markeredgecolor=st.get("markeredgecolor", "#000000"),
                    markeredgewidth=st.get("markeredgewidth", max(0.8 * FONT_SCALE, 0.5)),
                    color=st.get("color", None),
                    markevery=markevery,
                    label=esh.pretty_electrolyte_label(electrolyte),
                    alpha=0.95,
                )
                plotted_any = True
