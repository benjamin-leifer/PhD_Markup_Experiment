...
def parse_sample_list(file_path):
    ...
    try:
        ...
        name_column = None
        name_options = ['Name', 'Sample', 'Sample Name', 'ID', 'Cell ID', 'Cell Name', 'Identifier']
        ...
        if not name_column:
            return [], f"Excel file must contain a name column. Available columns: {', '.join(df.columns)}"
        ...
        column_mappings = {
            ...
            'nominal_capacity': ['Capacity', 'Nominal Capacity', 'Rated Capacity', 'Capacity (mAh)', 'Nominal Capacity (mAh)']
        }
...
def import_samples(file_path, update_existing=True):
    ...
    for sample_data in samples:
        try:
            ...
            existing = models.Sample.objects(name=sample_data['name']).first()
            ...
            logging.error(f"Error importing sample {sample_data.get('name', 'unknown')}: {str(e)}")
